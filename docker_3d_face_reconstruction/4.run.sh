...
INPUT="/data/dataset/vrn/examples/"           # Place to find input images
OUTPUT="/data/dataset/vrn/output/"            # Place to dump raw volumes
...
# import torch
if [ -d "/root/usr/local/torch/install/bin" ]; then
    export PATH=$PATH:/root/usr/local/torch/install/bin
fi
